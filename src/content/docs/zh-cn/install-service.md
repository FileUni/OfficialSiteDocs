...
安装服务时，必须通过 `--service-workdir` 传入唯一的运行目录，这个路径会被持久化写入服务定义。

```bash
# Linux/macOS 示例
# 使用 --service-workdir 指定服务安装的运行目录
...
> 重要提示：安装服务时请务必为 `--service-workdir` 使用绝对路径。这能确保系统重启后服务依然能正确定位到相关目录。
...
- `--service-workdir <目录>`：仅供 `service install` 使用的运行目录参数。

### 示例：自定义服务标签

```bash
sudo ./fileuni service install --service-workdir /srv/fileuni --service-label my.fileuni.node
```

## 常见问题

- 权限问题：安装系统级服务通常需要 Root (Linux/macOS) 或 管理员 (Windows) 权限。
- 路径要求：`service install` 必须配合有效的 `--service-workdir` 使用。
